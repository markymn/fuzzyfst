...
// Compiled Damerau-Levenshtein DFA.
//
// Before FST traversal, the full DFA is built from the standard
// Damerau-Levenshtein DP recurrence (including the transposition case
// D[i-2][j-2]+1).  Each DFA state maps to a unique DP column; transitions
// are precomputed for all 256 byte values.
//
