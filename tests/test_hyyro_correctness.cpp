// test_hyyro_correctness.cpp — Brute-force Damerau-Levenshtein oracle for Hyyro.
//
// Runs fuzzy_search with Hyyro's bit-parallel Damerau-Levenshtein NFA
// against a brute-force DP oracle and asserts identical result sets.
// This is the most important test for the Hyyro implementation.
...
// ── Run Hyyro fuzzy search via HyyroIterator ──

static std::set<std::pair<std::string, uint32_t>>
fst_hyyro(const FstReader& reader, std::string_view query, uint32_t max_dist) {
    HyyroNFA nfa;
    ...
    HyyroIterator iter(reader, nfa, word_buf, sizeof(word_buf),
                       result_buf, 8192);
...
static void test_small_dict_hyyro() {
    ...
    for (auto query : queries) {
        for (uint32_t d = 1; d <= 3; ++d) {
            ...
            auto got = fst_hyyro(reader, query, d);
    ...
    std::printf("  PASS: test_small_dict_hyyro (%zu queries x 3 distances)\n",
                queries.size());
}

// ── Test: real word list oracle (370K words) ──
...
static void test_real_word_list_hyyro() {
    auto dict = load_words("data/words.txt", SIZE_MAX);
    if (dict.empty()) {
        std::printf("  SKIP: test_real_word_list_hyyro (data/words.txt not found)\n");
        return;
    }

    auto reader = build_fst(dict);

    // 100 queries at d=1: mix of transposition-heavy and standard edits.
    std::vector<std::string_view> queries = {
        // Transposition queries (from spec)
        "teh", "freind", "wierd", "recieve", "abotu",
        "hte", "algortihm", "becuase", "accidnet",
        // Standard edit queries
        "helo", "algoritm", "aple", "acress", "adres",
        "agre", "wrld", "beutiful", "definately", "enviroment",
        "goverment", "harrass", "independant", "knowlege", "milennium",
        "neccessary", "occurence", "parliment", "restarant", "seperate",
        // More transpositions
        "taht", "thier", "waht", "wnat", "abuot",
        "ahve", "adn", "thn", "hte", "teh",
        // Common words
        "cat", "dog", "the", "and", "for",
        "run", "big", "lot", "hot", "sit",
        // Longer words
        "dictionary", "computer", "algorithm", "language", "software",
        "database", "function", "variable", "constant", "iterator",
        // Short words
        "a", "ab", "be", "do", "go",
        "if", "in", "is", "it", "me",
        // Misspellings
        "accomodate", "apparantly", "calender", "cemetary", "colleage",
        "concensus", "definite", "embarass", "firey", "florescent",
        "foriegn", "grammer", "humourous", "imediately", "jewellry",
        "liason", "mischievious", "noticable", "occassion", "perseverence",
        "priviledge", "questionaire", "refering", "sieze", "succesful",
        "tendancy", "threshhold", "tommorow", "truely", "unfortunatly",
        "untill", "wether", "wilfull", "withhold", "yeild",
        // Edge cases
        "", "x", "zz", "zzz", "abcdefghij",
    };
    ...
    for (auto query : queries) {
        ...
        auto got = fst_hyyro(reader, query, 1);

        if (got != expected) {
            std::fprintf(stderr,
                "FAIL: real word list hyyro: query=\"%.*s\" d=1: "
            ...
            int printed = 0;
            for (auto& [w, dist] : expected) {
                if (got.find({w, dist}) == got.end() && printed++ < 10)
                    std::fprintf(stderr, "  MISSING: \"%s\" (d=%u)\n", w.c_str(), dist);
            }
            for (auto& [w, dist] : got) {
                if (expected.find({w, dist}) == expected.end() && printed++ < 10)
                    std::fprintf(stderr, "  EXTRA:   \"%s\" (d=%u)\n", w.c_str(), dist);
            }
            assert(false);
        }
    ...
    // First 20 queries at d=2.
    for (size_t qi = 0; qi < 20 && qi < queries.size(); ++qi) {
        auto query = queries[qi];
        auto expected = brute_force_damerau(dict, query, 2);
        auto got = fst_hyyro(reader, query, 2);

        if (got != expected) {
            std::fprintf(stderr,
                "FAIL: real word list hyyro: query=\"%.*s\" d=2: "
                "expected %zu results, got %zu\n",
                static_cast<int>(query.size()), query.data(),
                expected.size(), got.size());
            int printed = 0;
    ...
    std::printf("  PASS: test_real_word_list_hyyro (%zu words, %zu query checks)\n",
...
void run_hyyro_correctness_tests() {
    std::printf("=== Hyyro Correctness Tests (Oracle) ===\n");
    test_small_dict_hyyro();
    test_real_word_list_hyyro();
    std::printf("=== All Hyyro correctness tests passed ===\n");
}
